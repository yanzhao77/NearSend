          ...
          child: ConstrainedBox(
            ...
            child: ListView(
              ...
              children: <Widget>[
                ...
                const NsInfoBanner(
                  title: '状态说明',
                  message: '协议版本为草案，尚未冻结。数据库 schema 与迁移代码已实现；运行时是否装配由上方状态明确标记。未注入 Git 或构建渠道时显示 unknown。',
                  tone: NsStatusTone.info,
                ),
                const SizedBox(height: NearSendSpacing.md),
                const NsInfoBanner(
                  title: '安全诊断',
                  message: '配对仍要求严格解析连接信息和 TLS 指纹校验。指纹变化会阻断连接；令牌、私钥、恢复密钥和文件内容不写入设置表或诊断文本。',
                  tone: NsStatusTone.success,
                ),
                const SizedBox(height: NearSendSpacing.md),
                const NsPermissionExplainer(
                  title: '平台验证状态',
                  message: '当前开发环境已验证 Android 构建、macOS 桌面和 Chrome；Windows、iOS 真机与平台存储能力需要对应环境验证，不能从这里推断为通过。',
                ),
              ],
            ),
          ),
