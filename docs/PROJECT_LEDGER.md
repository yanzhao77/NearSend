...
| T06 终检/导出/空间检查 | 部分完成：空间计划已实现（按卷明细、`unknown≠sufficient`、安全余量策略）；终检、导出与清理待做 | 保存失败可重试且不重复导出 |
...
| T06-01 空间计划、终检与导出 | T01-01/T04-01 | 进行中（空间计划已实现并测试：按卷明细、`unknown≠sufficient`、安全余量策略；终检、导出与清理未完成） | 分卷明细、unknown/不足、终检和幂等导出通过 |
