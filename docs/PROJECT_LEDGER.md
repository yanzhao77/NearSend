...
| T03 配对与单文件链路 | 部分完成：配对载荷、信任上下文、令牌生命周期、`/v1/pair` 模型、`/v1` 线上契约层与 §7 路由表已实现；端点逻辑与网络绑定未做 | 用户授权后端到端收发、鉴权负例通过 |
...
| T03-01 同网二维码配对 | T01-01/T02-02/**B02（仅网络绑定部分）** | 进行中（配对载荷、信任上下文、令牌生命周期与限流、`/v1/pair` 模型、`/v1` 线上契约层、**§7 的 19 行路由表与参数校验**已实现并测试；**目标端网络绑定仍阻塞于 B02**；19 个端点的实际逻辑、HTTP 服务器与 TLS 引擎未做） | pin 先于令牌，正负例和目标端网络绑定通过 |
...
| 分页的 1 MiB 约束 | §6：页「同时受 1 MiB 上限约束」 | **只做了条目数上限（`t03-01-04`）**：128/1024 已强制，但字节上限取决于条目实际大小，属响应构建阶段，**实现响应时必须补上** |
