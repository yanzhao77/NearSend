...
| T11-02 UI 装配 | 就绪 | 见 [T11-02](tasks/T11-02.md)：节点生命周期、身份说明、选择→会话、接收侧装配、真机第 6 项 |
...
#### 真机验证路径的阻塞（连续 11 次尝试，记录而不改写结论）

- `INSTALL_FAILED_USER_RESTRICTED` 自 `t03-02-05` 之后**每一次尝试都出现**（至今 **11 次**，全部为同一失败码），
...
- **解锁只需一个人工动作**：在设备上允许「通过 USB 安装应用」，或在该设备的开发者选项中开启
  ...
  `integration_test/android_bidirectional_transfer_test.dart` 的文件头中写全。
