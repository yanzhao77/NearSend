...
| T06 终检/导出/空间检查 | 部分完成：空间计划与整文件终检已实现（按卷明细、`unknown≠sufficient`、坏块降级为 `missing`）；导出与清理待做 | 保存失败可重试且不重复导出 |
...
| T06-01 空间计划、终检与导出 | T01-01/T04-01 | 进行中（空间计划与整文件终检已实现并测试：按卷明细、`unknown≠sufficient`、坏块降级为 `missing`；导出与清理未完成） | 分卷明细、unknown/不足、终检和幂等导出通过 |
...
| **终检与导出的强制关系** | 「完成 = 终检通过且导出结果已提交」必须由**代码**保证，不能只靠调用约定 | **缺口（未修，导出批次必须关闭）**：`TransferRepository.recordSavedExport` 只检查 committed 块数，**不要求先通过终检**；当所有块 committed 但整文件摘要不一致（清单级不一致）时该检查会通过。今天不可利用（转移引擎尚不存在、无生产调用方），但**导出批次必须让代码强制它**，见 [终检证据](testing/evidence/2026-09-20/t06-01-02/summary.md) §7 |
