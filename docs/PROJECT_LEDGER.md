...
| T03 配对与单文件链路 | 部分完成：配对载荷、信任上下文、令牌生命周期、`/v1/pair` 模型、`/v1` 线上契约层、§7 路由表、§6 清单分页/seal 与 §7 响应体已实现；端点逻辑与网络绑定未做 | 用户授权后端到端收发、鉴权负例通过 |
...
| T03-01 同网二维码配对 | T01-01/T02-02/**B02（仅网络绑定部分）** | 进行中（配对载荷、信任上下文、令牌生命周期与限流、`/v1/pair` 模型、`/v1` 线上契约层、**§7 的 19 行路由表与参数校验**、**§6 清单分页与 seal**、**§7 响应体** 已实现并测试；**目标端网络绑定仍阻塞于 B02**；§9 状态体、19 个端点的实际逻辑、HTTP 服务器与 TLS 引擎未做） | pin 先于令牌，正负例和目标端网络绑定通过 |
...
| §8 的块头规则 | 单一 `Content-Length`、不支持 `Transfer-Encoding`、不许压缩、同时出现必须拒绝、末尾多余数据拒绝并关闭连接 | **完全未实现（无任何代码覆盖）**：属 §7 端点与 HTTP 层，`t03-01-03`/`t03-01-04` 均未触及。实现 HTTP 传输时必须逐条落实并测试 |
