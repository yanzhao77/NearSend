...
| T03 配对与单文件链路 | 部分完成：配对载荷与信任上下文已实现；网络绑定阻塞于 B02 | 用户授权后端到端收发、鉴权负例通过 |
...
| T03-01 同网二维码配对 | T01-01/T02-02/**B02（仅网络绑定部分）** | 进行中（配对载荷解析、规范 base64url、DER 指纹计算、每连接信任上下文与「pin 先于令牌」已实现并测试；**目标端网络绑定仍阻塞于 B02**；令牌服务端生命周期与 `POST /v1/pair` 未做） | pin 先于令牌，正负例和目标端网络绑定通过 |
...
| `expiresInSeconds` 的权威性 | 协议 §3：二维码里的值是**提示**，以服务端状态为准 | `t03-01-01` 只校验它为正数，**不**把它当过期时间。后续实现配对令牌生命周期时不得改成信任它 |
