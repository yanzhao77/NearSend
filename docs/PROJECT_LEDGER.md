...
| T03 配对与单文件链路 | 部分完成：配对载荷、信任上下文、令牌生命周期、`/v1/pair` 模型与 `/v1` 线上契约层已实现；§7 端点与网络绑定未做 | 用户授权后端到端收发、鉴权负例通过 |
...
| T03-01 同网二维码配对 | T01-01/T02-02/**B02（仅网络绑定部分）** | 进行中（配对载荷、信任上下文、令牌生命周期与限流、`/v1/pair` 模型、`/v1` 线上契约层（严格 JSON 体／错误体／Bearer／Retry-After、§11 表被双向钉住）已实现并测试；**目标端网络绑定仍阻塞于 B02**；§7 的 16 个端点、HTTP 服务器与 TLS 引擎未做） | pin 先于令牌，正负例和目标端网络绑定通过 |
...
| §8 的块头规则 | 单一 `Content-Length`、不支持 `Transfer-Encoding`、不许压缩、同时出现必须拒绝、末尾多余数据拒绝并关闭连接 | **完全未实现（无任何代码覆盖）**：属 §7 端点与 HTTP 层，`t03-01-03` 未触及。实现 HTTP 传输时必须逐条落实并测试 |
