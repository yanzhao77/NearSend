...
| NearSend 1.2 升级 | **进行中**：V12-00 至 V12-11 均已有实现或明确阻塞记录；系统目录、接收确认、身份、发现、网络引导、二维码、雷达及系统文件动作已通过当前环境自动化；PAKE、指定 Android Network、Windows 自动入网和双机实测仍阻塞 | 执行 V12-12 回归/构建/证据收口，并在目标环境补 Android/Windows 双机、热点、系统文件动作和安全配对证据 |
