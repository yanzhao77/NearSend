...
| T03 配对与单文件链路 | 部分完成：配对载荷、信任上下文、令牌生命周期与 `/v1/pair` 模型已实现；网络绑定阻塞于 B02 | 用户授权后端到端收发、鉴权负例通过 |
...
| T03-01 同网二维码配对 | T01-01/T02-02/**B02（仅网络绑定部分）** | 进行中（配对载荷、规范 base64url、DER 指纹计算、每连接信任上下文与「pin 先于令牌」、令牌生命周期与限流、`/v1/pair` 模型已实现并测试；**目标端网络绑定仍阻塞于 B02**；HTTP 传输层与 TLS 引擎未做） | pin 先于令牌，正负例和目标端网络绑定通过 |
...
| 限流与熵的关系 | 协议 §3：**限流不替代随机熵** | `t03-01-02` 的限流只是纵深防御；`generatePairingTokenBytes` 必须保持 32 字节平台 CSPRNG 熵，**不得**因存在限流而降低。限流状态目前仅在进程内，重启清零（§3 未要求持久化，需真机确认该读法） |
