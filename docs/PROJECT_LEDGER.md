...
| T06 终检/导出/空间检查 | 部分完成：空间计划、整文件终检、导出门禁（导出必须带终检回执）与安全命名已实现；导出编排与清理待做 | 保存失败可重试且不重复导出 |
...
| T06-01 空间计划、终检与导出 | T01-01/T04-01 | 进行中（空间计划、整文件终检、导出门禁与安全命名已实现并测试；导出编排与清理未完成） | 分卷明细、unknown/不足、终检和幂等导出通过 |
...
| 目标已有条目的枚举完整性 | 导出不得覆盖用户已有文件；枚举不全时策略会认为名字空闲 | **本模块唯一会导致数据丢失的入口**：`takenPaths` 由调用方提供。`FileSink` 实现时必须明确「枚举失败 → 拒绝导出并请用户处理」，**不得**当作「没有冲突」。默认按大小写不敏感只降低概率，未消除该风险 |
