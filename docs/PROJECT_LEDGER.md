...
| T02 协议/模型/向量 | 部分完成 | 草案、Python 向量与 **Dart 独立实现**已逐字节一致（T02-01）；仍需完整错误模型/版本协商（T02-02）与原生实现比对才能冻结 |
...
| 协议冻结 | 双向角色、分页快照、恢复幂等与版本规则 | 草案的 LFTM1/LFTC1 已由 Dart 独立实现复现一致（T02-01）；**仍未冻结**，待 T02-02 错误模型/版本协商与原生实现比对 |
