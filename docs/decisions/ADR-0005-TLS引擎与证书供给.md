...
2. 用 `setTrustedCertificatesBytes` 装入**恰好那一张**由二维码携带的 pin 所对应的对端证书。
   ...
   不依赖我们自己的比较是否正确实现。
3. **仍然安装 `badCertificateCallback` 并恒返回 `false`**，作为 fail-closed 纵深防御：
   任何没能匹配上唯一信任证书的连接一律拒绝，而不是回落到某个默认行为。
4. 连接建立后，在 `SecureSocket.peerCertificate.der` 上**再显式比对一次**
   `SHA-256(DER)` 与 pin。这一步让协议 §2「必须比对 pin」这条规则在代码里有一个
   可读、可测、可断言的落点，也使任何「信任库被误配成宽松」的回归能被测试抓住。

三、四两步是有意的冗余：信任库保证「不是 pin 就连不上」，显式比对保证「规则本身被执行」，
两者失效原因不同，不构成同一条单点。

**离线时钟与主机名**：身份由 pin 唯一确定，证书有效期与主机名**不参与**判定。
协议 §2 已说明「不因离线时钟错误改成接受任意证书」；本决策的读法是**不依赖**它们，
而不是「忽略校验结果」。这一取舍已登记待确认。
