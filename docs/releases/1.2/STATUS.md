...
| V12-09 二维码入口 | 进行中 | 已定义严格 `nearsend-bootstrap` v1，绑定长期身份、TLS pin、一次性令牌、角色和可选真实热点凭据；旧 `lft-pair` 兼容，重复/额外字段失败关闭 | 审查并接入二维码生成、Android 离线扫码、Windows 系统图片导入解码及统一协调器 |
...
- 7 项定向测试通过。二维码图像生成、Android 摄像头离线扫码、Windows 图片导入解码和扫码后
  入网/认证协调尚未实现，V12-09 不能标记完成。
