...
| V12-09 二维码入口 | 进行中 | 严格 `nearsend-bootstrap` v1、旧码兼容、真实会话二维码、Android/iOS 离线摄像头组件及 Windows 有界图片导入已实现；全量 1353 项通过，iOS Simulator 构建通过 | 接入扫码后的热点入网/TLS 配对协调器；补 Android/Windows 构建与平台实测 |
...
- 7 项载荷测试通过。扫码后入网/认证协调尚未实现，V12-09 不能标记完成。
